...
- "跟 claude 相關星星數最多的 repo" -> claude in:name,description
- "react 最多星的函式庫" -> language:javascript react library in:name,description
...
- "most stars" / "top starred" / "最多星" / "星星數最多" / "星數最高" WITHOUT a numeric threshold:
  do NOT emit any stars qualifier. GitHub search already returns results sorted by stars desc;
  adding `stars:>0` is a no-op that clutters the query. Only add stars: when the user gave a NUMBER.
- "related to X" / "X 相關" / "跟 X 相關" where X is a product/tool/library name:
  append `in:name,description` so matches are restricted to repo name and description and don't
  pull in tangential README mentions. Example: "跟 claude 相關" -> claude in:name,description
